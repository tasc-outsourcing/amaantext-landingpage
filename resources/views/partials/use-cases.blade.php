<section id="use-cases" class="py-32 px-8 lg:px-16 bg-slate-50">
    <div class="container mx-auto max-w-7xl">
        ...
        <div class="text-center mb-12">
            ...
            {{-- Role Pills --}}
            <div class="flex justify-center gap-3 max-w-full mx-auto overflow-x-auto pb-2">
                @foreach([
                ...
                <div
                    data-role-pill="{{ $index }}"
                    class="px-5 py-2.5 rounded-full text-[14px] font-medium transition-all cursor-pointer whitespace-nowrap flex-shrink-0 {{ $index === 0 ? 'bg-[#005f83] text-white shadow-lg' : 'bg-white text-slate-600 border border-slate-300 hover:border-[#005f83] hover:text-[#005f83]' }}"
                >
                    {{ $role }}
                </div>
                @endforeach
        ...
        <div class="max-w-4xl mx-auto mb-16">
            ...
            <div data-role-card="{{ $index }}" class="group relative bg-white rounded-[24px] p-8 border-2 border-slate-200 shadow-lg transition-all duration-300 {{ $index === 0 ? '' : 'hidden' }}">
                ...
                <div class="flex gap-6 mb-6 items-center">
                    ...
                    <div class="relative flex-shrink-0">
                        ...
                        <div class="relative w-20 h-20 rounded-full bg-[#005f83] flex items-center justify-center text-white shadow-lg">
                    ...
                    {{-- Role Title --}}
                    <div class="flex-1">
                        <h3 class="text-[26px] font-bold text-[#005f83] font-['Poppins',sans-serif] leading-tight">
                            {{ $item['role'] }}
                        </h3>
                    </div>

                    {{-- Badge --}}
                    <div class="flex-shrink-0 h-fit px-4 py-2 bg-[#ffe102] rounded-full">
                        <svg class="w-6 h-6 text-[#005f83]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="mb-6">
                    <div class="flex items-center gap-2 mb-3">
                        ...
                        <span class="text-[13px] font-bold text-[#005f83] uppercase tracking-wider">The Challenge</span>
                    </div>
                    <p class="text-[15px] text-slate-700 leading-relaxed italic font-['Poppins',sans-serif]">
                        "{{ $item['challenge'] }}"
                    </p>
                </div>

                {{-- Solution --}}
                <div class="mb-6 bg-[#005f83] rounded-[16px] p-5">
                    <div class="flex items-start gap-3">
                        <div class="flex-shrink-0 w-8 h-8 bg-[#ffe102] rounded-lg flex items-center justify-center">
                            <svg class="w-5 h-5 text-[#005f83]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                {{-- Documents --}}
                <div>
                    <div class="text-[12px] font-bold text-slate-500 uppercase tracking-wider mb-3">Document Types</div>
                    <div class="flex flex-wrap gap-2">
                        @foreach($item['documents'] as $doc)
                        <div class="px-3 py-1.5 bg-slate-100 rounded-full text-[12px] font-semibold text-slate-700 border border-slate-200 hover:bg-[#ffe102]/20 hover:border-[#ffe102] transition-colors">
                            {{ $doc }}
                        </div>
                        @endforeach
                    </div>
        ...
        {{-- Bottom CTA Bar --}}
        <div class="bg-[#005f83] rounded-[24px] p-10 text-center shadow-xl">
            <h3 class="text-[32px] font-bold text-white mb-4 font-['Poppins',sans-serif]">
